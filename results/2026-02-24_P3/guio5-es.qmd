...
fields <- 'fields=tax_id,scientific_name,base_count,first_public,description&'
limit  <- 'limit=0'
URL <- paste0(portal, result, query, fields, limit)
curl_download(URLencode(URL), destfile = 'droph.txt')
```
#lo unico que hemos hecho para que nos saliera es cambiar, en results, en lugar de poner = coding, poner sequence, y añadir en fields, que queremos que nos de la descripcion, de modo que ahora si que nos salen datos, por lo que tenemos que ver si todos hacen referecia a la alcholo deshidrogenasa o solo tienen relacion de algun tipo con ella,  pero no presentan su secuencia. 
:::
...
#se puede saber bajando hasta bajo y viendo el numero de lineas que tiene el documento. (hemos de tener en cuenta que la primera linea son los nombres de las columnas por lo que esa no cuenta). en este caso contamos con 1258 secuencias. 
2. ¿Qué longitudes tienen? 
#base_count es la que nos dice la longitud de cada secuencia, indica el numero de nt de cada secuencia. 
#tabla$base_count si ponemos este comando, nos hace una tabla solo con los datos de base_count. 
3. ¿De cuántas especies diferentes son estas secuencias?
#ahora hay que hacer lo msimo que con base_count, pero con scientific_name
#308g
